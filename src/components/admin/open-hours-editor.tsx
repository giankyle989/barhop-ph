...
export function OpenHoursEditor({ value, onChange }: OpenHoursEditorProps) {
  ...
  return (
    <div className="w-full rounded bg-surface-card border border-border overflow-hidden">
      {/* Header — desktop only */}
      <div className="hidden sm:grid grid-cols-[100px_1fr_auto_1fr_auto] gap-3 px-4 py-2 border-b border-border bg-surface-raised">
      ...
      {DAYS.map(({ key, label }, index) => {
        ...
        return (
          <div
            key={key}
            className={`${!isLast ? "border-b border-border" : ""} ${isClosed ? "opacity-60" : ""}`}
          >
            {/* Desktop layout */}
            <div className="hidden sm:grid grid-cols-[100px_1fr_auto_1fr_auto] gap-3 items-center px-4 py-3">
            ...
            <div className="sm:hidden px-4 py-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-content-muted">Closed</span>
                  <Toggle
                    label={`${label} closed`}
                    checked={isClosed}
                    onChange={(checked) => handleClosedToggle(key, checked)}
